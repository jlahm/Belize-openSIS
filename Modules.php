...
$schools = substr(str_replace(",","','",User('SCHOOLS')),2,-2);
								...
								echo "<SELECT name=school onChange='this.form.submit();' style='width:150;'>";
...
$school_years_RET1=DBGet(DBQuery("SELECT START_DATE,END_DATE FROM SCHOOL_YEARS WHERE SCHOOL_ID=".UserSchool()));
 ...
 echo "<SELECT name=syear onChange='this.form.submit();' style='width:150;'>";
						#foreach($years_RET as $year)
                                                  ...
                                                  if($school_years_RET1['END_DATE']==$school_years_RET1['START_DATE']+1)
                                                   {
                                                         ...
                                                         echo "SELECT START_DATE,END_DATE FROM SCHOOL_YEARS sy,STAFF s WHERE s.SYEAR=sy.SYEAR AND SCHOOL_ID=".UserSchool()." AND  s.USERNAME=(SELECT USERNAME FROM STAFF WHERE SYEAR='$DefaultSyear' AND STAFF_ID='$_SESSION[STAFF_ID]')";

                                                    echo  $school_years_RET=DBGet(DBQuery("SELECT START_DATE,END_DATE FROM SCHOOL_YEARS sy,STAFF s WHERE s.SYEAR=sy.SYEAR AND SCHOOL_ID=".UserSchool()." AND  s.USERNAME=(SELECT USERNAME FROM STAFF WHERE SYEAR='$DefaultSyear' AND STAFF_ID='$_SESSION[STAFF_ID]')"));
					               ...
					               echo $school_years['END_DATE']=$school_years['END_DATE'][0];
                                                    ...
                                                    else if($school_years_RET1['END_DATE']==$school_years_RET1['START_DATE'])
                                                   {
                                                       ...
                                                       if(UserSchool())
                                                       $school_years_RET=DBGet(DBQuery("SELECT START_DATE,END_DATE FROM SCHOOL_YEARS sy ,STAFF s WHERE s.SYEAR=sy.SYEAR  and sy.SCHOOL_ID=".UserSchool()." AND s.USERNAME=(SELECT USERNAME FROM STAFF WHERE SYEAR='$DefaultSyear' AND STAFF_ID='$_SESSION[STAFF_ID]')"));
                                                       else
                                                       $school_years_RET=DBGet(DBQuery("SELECT START_DATE,END_DATE FROM SCHOOL_YEARS sy ,STAFF s WHERE s.SYEAR=sy.SYEAR  AND s.USERNAME=(SELECT USERNAME FROM STAFF  WHERE SYEAR='$DefaultSyear' AND STAFF_ID='$_SESSION[STAFF_ID]')"));
...
echo "<SELECT name=mp onChange='this.form.submit();' style='width:150;'>";
if(count($RET))
{
	if(!UserMP())
	...
	foreach($RET as $quarter)
	{
			...
			if(UserMP()==$quarter['COURSE_PERIOD_ID'])
			{
				//$_SESSION['UserSchool'] = $period['SCHOOL_ID'];
				//$_SESSION['UserSubject'] = $period['SUBJECT_ID'];
				//$_SESSION['UserCourse'] = $period['COURSE_ID'];
...
$sub = DBQuery("SELECT DISTINCT cs.TITLE, cs.SUBJECT_ID, cd.TEACHER_ID,cs.SCHOOL_ID FROM COURSE_SUBJECTS as cs,COURSE_DETAILS as cd WHERE cs.SUBJECT_ID=cd.SUBJECT_ID AND cd.SYEAR='".UserSyear()."' AND (cd.TEACHER_ID='".User('STAFF_ID')."' OR cd.SECONDARY_TEACHER_ID='".User('STAFF_ID')."') AND cs.SCHOOL_ID='".UserSchool()."' AND cd.MARKING_PERIOD_ID IN (".GetAllMP($allMP,UserMP()).")");
					   ...
					   echo "<SELECT name=subject onChange='this.form.submit();' style='width:150;'>";
						foreach($RET as $subject)
						{
						  ...
						  if(UserSubject()==$subject['SUBJECT_ID'])
						  {
						
							//$_SESSION['UserSchool'] = $period1['SCHOOL_ID'];
							$_SESSION['UserSubject'] = $subject['SUBJECT_ID'];
							//$_SESSION['UserPeriod'] = $period1['PERIOD_ID'];
							//unset($_SESSION['UserCourse']);
							//unset($_SESSION['UserCoursePeriod']);
...
$course = DBQuery("SELECT DISTINCT cd.COURSE_TITLE, cd.COURSE_ID,cd.SUBJECT_ID,cd.SCHOOL_ID FROM COURSE_DETAILS cd WHERE (cd.TEACHER_ID='".User('STAFF_ID')."' OR cd.SECONDARY_TEACHER_ID='".User('STAFF_ID')."') AND cd.SYEAR='".UserSyear()."' AND cd.SCHOOL_ID='".UserSchool()."' AND cd.SUBJECT_ID='".UserSubject()."' AND cd.MARKING_PERIOD_ID IN (".GetAllMP($allMP,UserMP()).")");
					...
					$RET = DBGet($course);
					  ...
					  {
					   		//echo "hiiii";
							//echo $RET[1]['COURSE_ID'];
							$_SESSION['UserCourse']=$RET[1]['COURSE_ID'];
					  }
					   echo "<SELECT name=course onChange='this.form.submit();' style='width:150;'>";
						foreach($RET as $course)
						{
						  ...
						  if(UserCourse()==$course['COURSE_ID'])
						  {
						
							//$_SESSION['UserSchool'] = $period2['SCHOOL_ID'];
							//$_SESSION['UserSubject'] = $period2['SUBJECT_ID'];
							$_SESSION['UserCourse'] = $course['COURSE_ID'];
							//$_SESSION['UserPeriod'] = $period2['PERIOD_ID'];
							//unset($_SESSION['UserCoursePeriod']);
...
//echo "SELECT DISTINCT cp.PERIOD_ID,cp.COURSE_PERIOD_ID,sp.TITLE,sp.SHORT_NAME,cp.MARKING_PERIOD_ID,cp.DAYS,sp.SORT_ORDER,c.TITLE AS COURSE_TITLE FROM COURSE_PERIODS cp, SCHOOL_PERIODS sp,COURSES c WHERE c.COURSE_ID=cp.COURSE_ID AND cp.PERIOD_ID=sp.PERIOD_ID AND cp.SYEAR='".UserSyear()."' AND cp.SCHOOL_ID='".UserSchool()."' AND cp.TEACHER_ID='".UserStaffID()."' ORDER BY sp.SORT_ORDER ";						
						...
						{
							//echo "hello";
							//echo  $RET[1]['COURSE_PERIOD_ID'];
							$_SESSION['UserCoursePeriod'] = $RET[1]['COURSE_PERIOD_ID'];
						}	
						
    					echo "<SELECT name=period onChange='this.form.submit();' style='width:150;'>";
...
{				
				...
				echo "<FORM name=head_frm id=head_frm action=Side.php?modfunc=update&btnn=$btn&nsc=$ns method=POST>
