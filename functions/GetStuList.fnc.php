	...
	switch(User('PROFILE'))
	{
		case 'admin':
			...
			else
			{
				...
				$sql .='s.LAST_NAME,s.FIRST_NAME,s.MIDDLE_NAME,s.STUDENT_ID,s.PHONE,ssm.SCHOOL_ID,s.ALT_ID,ssm.SCHOOL_ID AS LIST_SCHOOL_ID,ssm.GRADE_ID,s.PHONE '.$extra['SELECT'];
		...
		$sql.=$extra['FROM']." WHERE ssm.STUDENT_ID=s.STUDENT_ID ";
			if($_REQUEST['include_inactive']=='Y')
				$sql .= " AND ssm.ID=(SELECT ID FROM STUDENT_ENROLLMENT WHERE STUDENT_ID=ssm.STUDENT_ID AND SYEAR <='".UserSyear()."' ORDER BY START_DATE DESC LIMIT 1)";
				 ...
				 $sql .= " AND ssm.SYEAR='".UserSyear()."' AND ((ssm.START_DATE IS NOT NULL AND '".date('Y-m-d',strtotime($extra['DATE']))."'>=ssm.START_DATE) AND ('".date('Y-m-d',strtotime($extra['DATE']))."'<=ssm.END_DATE OR ssm.END_DATE IS NULL)) ";
...
if($_REQUEST['reason'] || $_REQUEST['result'] || $_REQUEST['med_vist_comments']||  $_REQUEST['nv_day'] || $_REQUEST['nv_month'] || $_REQUEST['nv_year']){
			...
			else
			{
				...
				$sql .= " AND (ssm.START_DATE IS NOT NULL AND ('".$extra['DATE']."'<=ss.END_DATE OR ss.END_DATE IS NULL))";
...
{	global $_openSIS;
         ...
         if($_REQUEST['altid'])
	{
		...
		$sql .= " AND LOWER(s.ALT_ID) LIKE '".strtolower(trim($_REQUEST['altid']))."%' ";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Student ID: </b></font>'.$_REQUEST['stuid'].'<BR>';
	}
	if($_REQUEST['last'])
	{
		$sql .= " AND LOWER(s.LAST_NAME) LIKE '".strtolower(trim($_REQUEST['last']))."%' ";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Last Name starts with: </b></font>'.trim($_REQUEST['last']).'<BR>';
	}
	if($_REQUEST['first'])
	{
		$sql .= " AND LOWER(s.FIRST_NAME) LIKE '".strtolower(trim($_REQUEST['first']))."%' ";
		if(!$extra['NoSearchTerms'])
	...
	if($_REQUEST['addr'])
	{
		$sql .= " AND (LOWER(a.ADDRESS) LIKE '%".strtolower(trim($_REQUEST['addr']))."%' OR LOWER(a.CITY) LIKE '".strtolower(trim($_REQUEST['addr']))."%' OR LOWER(a.STATE)='".strtolower(trim($_REQUEST['addr']))."' OR ZIPCODE LIKE '".trim($_REQUEST['addr'])."%')";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Address contains: </b></font>'.trim($_REQUEST['addr']).'<BR>';
	}
	if($_REQUEST['preferred_hospital'])
	{
		$sql .= " AND LOWER(s.PREFERRED_HOSPITAL) LIKE '".strtolower($_REQUEST['preferred_hospital'])."%' ";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Preferred Medical Facility starts with: </b></font>'.$_REQUEST['preferred_hospital'].'<BR>';
	}
	if($_REQUEST['mp_comment'])
	{
		$sql .= " AND LOWER(smc.COMMENT) LIKE '".strtolower($_REQUEST['mp_comment'])."%' AND s.STUDENT_ID=smc.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Comments starts with: </b></font>'.$_REQUEST['mp_comment'].'<BR>';
	}
	if($_REQUEST['goal_title'])
	{
		$sql .= " AND LOWER(g.GOAL_TITLE) LIKE '".strtolower($_REQUEST['goal_title'])."%' AND s.STUDENT_ID=g.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Goal Title starts with: </b></font>'.$_REQUEST['goal_title'].'<BR>';
	}
		if($_REQUEST['goal_description'])
	{
		$sql .= " AND LOWER(g.GOAL_DESCRIPTION) LIKE '".strtolower($_REQUEST['goal_description'])."%' AND s.STUDENT_ID=g.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
	...
	{
		$sql .= " AND LOWER(p.PROGRESS_NAME) LIKE '".strtolower($_REQUEST['progress_name'])."%' AND s.STUDENT_ID=p.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
	...
	{
		$sql .= " AND LOWER(p.PROGRESS_DESCRIPTION) LIKE '".strtolower($_REQUEST['progress_description'])."%' AND s.STUDENT_ID=p.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
	...
	{
		$sql .= " AND LOWER(smn.DOCTORS_NOTE_COMMENTS) LIKE '".strtolower($_REQUEST['doctors_note_comments'])."%' AND s.STUDENT_ID=smn.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Doctor\'s Note starts with: </b></font>'.$_REQUEST['doctors_note_comments'].'<BR>';
	}
	if($_REQUEST['type'])
	{
		$sql .= " AND LOWER(sm.TYPE) LIKE '".strtolower($_REQUEST['type'])."%' AND s.STUDENT_ID=sm.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Type starts with: </b></font>'.$_REQUEST['type'].'<BR>';
	}
	if($_REQUEST['imm_comments'])
	{
		$sql .= " AND LOWER(sm.COMMENTS) LIKE '".strtolower($_REQUEST['imm_comments'])."%' AND s.STUDENT_ID=sm.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
...
$nv_date=$_REQUEST['nv_year'].'-'.$_REQUEST['nv_month'].'-'.$_REQUEST['nv_day'];
	...
	{
		$sql .= " AND LOWER(sma.TITLE) LIKE '".strtolower($_REQUEST['med_alrt_title'])."%' AND s.STUDENT_ID=sma.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Alert starts with: </b></font>'.$_REQUEST['med_alrt_title'].'<BR>';
	}
	if($_REQUEST['reason'])
	{
		$sql .= " AND LOWER(smv.REASON) LIKE '".strtolower($_REQUEST['reason'])."%' AND s.STUDENT_ID=smv.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Reason starts with: </b></font>'.$_REQUEST['reason'].'<BR>';
	}
	if($_REQUEST['result'])
	{
		$sql .= " AND LOWER(smv.RESULT) LIKE '".strtolower($_REQUEST['result'])."%' AND s.STUDENT_ID=smv.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>Result starts with: </b></font>'.$_REQUEST['result'].'<BR>';
	}
	if($_REQUEST['med_vist_comments'])
	{
		$sql .= " AND LOWER(smv.COMMENTS) LIKE '".strtolower($_REQUEST['med_vist_comments'])."%' AND s.STUDENT_ID=smv.STUDENT_ID ";
		if(!$extra['NoSearchTerms'])
