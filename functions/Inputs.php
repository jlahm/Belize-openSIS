...
function SelectInput_for_EndInput($value,$name,$title='',$options,$allow_na='N/A',$extra='',$div=true)
{
	if(Preferences('HIDDEN')!='Y')
		$div = false;
	...
	if(AllowEdit() && !$_REQUEST['_openSIS_PDF'])
	{
		...
		$return .= "<SELECT name=$name $extra  onchange=javascript:if(this.value==\"5\")window.open(\"for_window.php?modname=$_REQUEST[modname]&modfunc=detail&student_id=".UserStudentID()."\",\"blank\",\"width=500,height=300\");>";
		if($allow_na!==false)
		{
			if($value!='' && $div)
				$return .= '<OPTION value=\"\">'.$allow_na;
