...
if($_REQUEST['modfunc']=='detail' && $_REQUEST['student_id'] && $_REQUEST['student_id']!='new')
{
	if($_POST['button']=='Save' && AllowEdit())
	{
		...
		DBQuery("UPDATE STUDENT_ENROLLMENT SET DROP_CODE=5,END_DATE='".$_REQUEST['TRANSFER']['STUDENT_ENROLLMENT_END_DATE']."' WHERE STUDENT_ID=".$_REQUEST['student_id']." AND SCHOOL_ID=".UserSchool()."  AND SYEAR=".UserSyear());
	...
	$syear_RET= DBGet(DBQuery("SELECT SYEAR,TITLE FROM SCHOOLS WHERE ID=".$_REQUEST['TRANSFER']['SCHOOL']));
	...
	if($gread_exists[1]['PRESENT']==1 && $gread_exists[1]['ID']){
         DBQuery("INSERT INTO STUDENT_ENROLLMENT (SYEAR ,SCHOOL_ID ,STUDENT_ID ,GRADE_ID ,START_DATE ,END_DATE ,ENROLLMENT_CODE ,DROP_CODE ,NEXT_SCHOOL ,CALENDAR_ID ,LAST_SCHOOL) VALUES (".$syear.",".$_REQUEST['TRANSFER']['SCHOOL'].",".$_REQUEST['student_id'].",".$gread_exists[1]['ID'].",'".$_REQUEST['TRANSFER']['STUDENT_ENROLLMENT_START']."','',6,'','".$_REQUEST['TRANSFER']['SCHOOL']."',NULL,$last_school)");
    }else{   
             DBQuery("INSERT INTO STUDENT_ENROLLMENT (SYEAR ,SCHOOL_ID ,STUDENT_ID ,GRADE_ID ,START_DATE ,END_DATE ,ENROLLMENT_CODE ,DROP_CODE ,NEXT_SCHOOL ,CALENDAR_ID ,LAST_SCHOOL) VALUES (".$syear.",".$_REQUEST['TRANSFER']['SCHOOL'].",".$_REQUEST['student_id'].",'','".$_REQUEST['TRANSFER']['STUDENT_ENROLLMENT_START']."','',6,'','".$_REQUEST['TRANSFER']['SCHOOL']."',NULL,$last_school)");
        }
	...
	else
	{
		...
		echo "<FORM name=popform id=popform action=for_window.php?modname=$_REQUEST[modname]&modfunc=detail&student_id=".UserStudentID()." METHOD=POST>";
		echo '<TABLE>';
...
if($_REQUEST['category_id']==5 && !isset($_REQUEST['goal_id']))
	{
	$goal_id = DBGet(DBQuery("SELECT GOAL_ID FROM GOAL WHERE STUDENT_ID='".UserStudentID()."'"));
    ...
    if((count($_REQUEST['students']) || count($_REQUEST['values'])) && AllowEdit())
    {
		if($_REQUEST['student_id'] && $_REQUEST['student_id']!='new')
        {
			...
			if(count($_REQUEST['students']))
            {
				...
				$custom_TITLE=DBGet(DBQuery("SELECT TITLE FROM CUSTOM_FIELDS WHERE ID=".$custom_id));
...
if($_REQUEST['stuid'])
	{
		$select .= " AND ssm.STUDENT_ID = '$_REQUEST[stuid]' ";
		
	}
   if($_REQUEST['altid'])
	{
		$select .= " AND s.ALT_ID = '$_REQUEST[altid]' ";
		
	}
   if($_REQUEST['last'])
	{
		$select .= " AND LOWER(s.LAST_NAME) LIKE '".strtolower(trim($_REQUEST['last']))."%' ";
		
	}
   if($_REQUEST['first'])
	{
		$select .= " AND LOWER(s.FIRST_NAME) LIKE '".strtolower(trim($_REQUEST['first']))."%' ";
		
	}
	if($_REQUEST['grade'])
	{
		$select .= " AND ssm.GRADE_ID = '$_REQUEST[grade]' ";
		
	}
	if($_REQUEST['addr'])
		{
		$select .= " AND (LOWER(a.ADDRESS) LIKE '%".strtolower(trim($_REQUEST['addr']))."%' OR LOWER(a.CITY) LIKE '".strtolower(trim($_REQUEST['addr']))."%' OR LOWER(a.STATE)='".strtolower(trim($_REQUEST['addr']))."' OR ZIPCODE LIKE '".trim($_REQUEST['addr'])."%')";
	...
	{
		$select .= " AND LOWER(smc.COMMENT) LIKE '".strtolower($_REQUEST['mp_comment'])."%' AND s.STUDENT_ID=smc.STUDENT_ID ";
	}
	if($_REQUEST['goal_title'])
	{
		$select .= " AND LOWER(g.GOAL_TITLE) LIKE '".strtolower($_REQUEST['goal_title'])."%' AND s.STUDENT_ID=g.STUDENT_ID ";
	}
		if($_REQUEST['goal_description'])
	{
		$select .= " AND LOWER(g.GOAL_DESCRIPTION) LIKE '".strtolower($_REQUEST['goal_description'])."%' AND s.STUDENT_ID=g.STUDENT_ID ";
	}
		if($_REQUEST['progress_name'])
	{
		$select .= " AND LOWER(p.PROGRESS_NAME) LIKE '".strtolower($_REQUEST['progress_name'])."%' AND s.STUDENT_ID=p.STUDENT_ID ";
	}
	if($_REQUEST['progress_description'])
	{
		$select .= " AND LOWER(p.PROGRESS_DESCRIPTION) LIKE '".strtolower($_REQUEST['progress_description'])."%' AND s.STUDENT_ID=p.STUDENT_ID ";
	}
	if($_REQUEST['doctors_note_comments'])
	{
		$select .= " AND LOWER(smn.DOCTORS_NOTE_COMMENTS) LIKE '".strtolower($_REQUEST['doctors_note_comments'])."%' AND s.STUDENT_ID=smn.STUDENT_ID ";
	}
	if($_REQUEST['type'])
	{
		$select .= " AND LOWER(sm.TYPE) LIKE '".strtolower($_REQUEST['type'])."%' AND s.STUDENT_ID=sm.STUDENT_ID ";
	}
	if($_REQUEST['imm_comments'])
	{
		$select .= " AND LOWER(sm.COMMENTS) LIKE '".strtolower($_REQUEST['imm_comments'])."%' AND s.STUDENT_ID=sm.STUDENT_ID ";
	}
...
$nv_date=$_REQUEST['nv_year'].'-'.$_REQUEST['nv_month'].'-'.$_REQUEST['nv_day'];
	...
	{
		$select .= " AND LOWER(sma.TITLE) LIKE '".strtolower($_REQUEST['med_alrt_title'])."%' AND s.STUDENT_ID=sma.STUDENT_ID ";
	}
	if($_REQUEST['reason'])
	{
		$select .= " AND LOWER(smv.REASON) LIKE '".strtolower($_REQUEST['reason'])."%' AND s.STUDENT_ID=smv.STUDENT_ID ";
	}
	if($_REQUEST['result'])
	{
		$select .= " AND LOWER(smv.RESULT) LIKE '".strtolower($_REQUEST['result'])."%' AND s.STUDENT_ID=smv.STUDENT_ID ";
	}
	if($_REQUEST['med_vist_comments'])
	{
		$select .= " AND LOWER(smv.COMMENTS) LIKE '".strtolower($_REQUEST['med_vist_comments'])."%' AND s.STUDENT_ID=smv.STUDENT_ID ";
	}
...
if(UserStudentID() || $_REQUEST['student_id']=='new')
{
	if($_REQUEST['student_id']!='new')
	{
		...
		$s1_id=DBGet(DBQuery("SELECT s.STUDENT_ID ".$admin_COMMON.$_SESSION['s']." ORDER BY LAST_NAME ASC LIMIT 1"));
		$s2_id=DBGet(DBQuery("SELECT s.STUDENT_ID ".$admin_COMMON.$_SESSION['s']." ORDER BY LAST_NAME DESC LIMIT 1"));
		
		$count_STU=DBGet(DBQuery("SELECT COUNT(LAST_NAME) AS STUDENT ".$admin_COMMON." AND LAST_NAME<'".$ln."' AND LAST_NAME LIKE '".strtolower($_REQUEST['last'])."%'".$_SESSION['s']));
		$count=$count_STU[1]['STUDENT'] + 1;
		$total=DBGet(DBQuery("SELECT COUNT(s.STUDENT_ID) AS STUDENT_ID ".$admin_COMMON." ".$_SESSION['s']));
		  }
		  elseif(User('PROFILE')=='teacher')
		   {
...
SCHEDULE ss
 WHERE s.STUDENT_ID=ssm.STUDENT_ID AND ssm.STUDENT_ID=ss.STUDENT_ID AND ssm.SYEAR=cp.SYEAR AND ssm.SYEAR=ss.SYEAR AND cp.COURSE_ID=ss.COURSE_ID AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.MARKING_PERIOD_ID IN (".GetAllMP('',$queryMP).")
		...
		$s1_id=DBGet(DBQuery("SELECT s.STUDENT_ID ".$teacher_COMMON.$_SESSION['s']." ORDER BY LAST_NAME ASC LIMIT 1"));
		
		$s2_id=DBGet(DBQuery("SELECT s.STUDENT_ID ".$teacher_COMMON.$_SESSION['s']." ORDER BY LAST_NAME DESC LIMIT 1"));
		
		$count_STU=DBGet(DBQuery("SELECT COUNT(LAST_NAME) AS STUDENT ".$teacher_COMMON." AND LAST_NAME<'".$ln."' AND LAST_NAME LIKE '".strtolower($_REQUEST['last'])."%'".$_SESSION['s']));
		$count=$count_STU[1]['STUDENT']+1;
				
			$total=DBGet(DBQuery("SELECT COUNT(s.STUDENT_ID) AS STUDENT_ID ".$teacher_COMMON." ".$_SESSION['s']));	
		   }
