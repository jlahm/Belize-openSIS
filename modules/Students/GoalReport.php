...
$sql_student = DBGet(DBQuery("SELECT gender AS GENDER, ethnicity AS ETHNICITY, common_name AS COM_NAME, social_security AS SOCIAL_SEC, language AS LANG, birthdate AS BDATE  FROM STUDENTS WHERE STUDENT_ID='".$_SESSION['student_id']."'"));

$sql_student = $sql_student[1];

$bd = $sql_student['BDATE'];
$bir_dt = show_date($bd);
...
{
	$sql_goal = "SELECT * FROM GOAL WHERE STUDENT_ID='".$_SESSION['student_id']."' AND SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."' ORDER BY GOAL_TITLE";
}
if(isset($st_dt) && !isset($end_dt))
{
	$sql_goal = "SELECT * FROM GOAL WHERE STUDENT_ID='".$_SESSION['student_id']."' AND SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."' AND START_DATE>='".$st_dt."' ORDER BY GOAL_TITLE";
}
if(!isset($st_dt) && isset($end_dt))
{
	$sql_goal = "SELECT * FROM GOAL WHERE STUDENT_ID='".$_SESSION['student_id']."' AND SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."' AND START_DATE<='".$end_dt."' ORDER BY GOAL_TITLE";
}
if(isset($st_dt) && isset($end_dt))
{
	$sql_goal = "SELECT * FROM GOAL WHERE STUDENT_ID='".$_SESSION['student_id']."' AND SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."' AND START_DATE>='".$st_dt."' AND START_DATE<='".$end_dt."' ORDER BY GOAL_TITLE";
}

$res_goal = mysql_query($sql_goal);
$count_goal = mysql_num_rows($res_goal);
	...
	#echo "<tr><td valign=top width=300><table width=100% ><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-weight:bold;\">Personal Information</td></tr>";
		...
		if($count_goal != 0)
		{
			...
			*/
			...
			echo "<tr><td colspan=2 style=\"height:18px\"></td></tr>";
			while($row_goal=mysql_fetch_array($res_goal))
			{
				$goal_id = $row_goal[0];
				
				$gst_dt = $row_goal[3];
				$gole_st_dt = show_date($gst_dt);
