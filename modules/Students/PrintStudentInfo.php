...
echo "<table width=100% ><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-size:14px;  font-weight:bold;\">Personal Information</td></tr>";
	...
	if($_REQUEST['category']['1'])
			...
			echo "<td>".show_date($sql['BIRTHDAY']) ."</td></tr>";
			if($sql['LANGUAGE'] !='')
			{
...
echo "<tr><td valign=top colspan=3>";
 ...
 #echo '</td><td></td><td></td></tr></table></TABLE><div style="page-break-before: always;">&nbsp;</div>';
			...
			if($_REQUEST['category']['2'] && ($sql['PHYSICIAN_NAME'] !='' || $sql['PHYSICIAN_PHONO'] !='' || $sql['HOSPITAL'] !=''))
			{
				...
				echo "<td align='justify'>".show_date($medical['MCOMNT']) ."</td></tr>";
				}
				if($medical['DNOTE'] !='')
				{
 ...
 #############################################################################
			...
			$sql_immunization = "select * from STUDENT_MEDICAL where student_id='".$_SESSION['student_id']."'";
			$res_immunization =mysql_query($sql_immunization);
			$num_immunization = mysql_num_rows($res_immunization);

			if($_REQUEST['category']['2'] && $num_immunization >= 1)
			{
				//------------------------------------------------------------------------------
				#echo "<br>";
				#echo $_REQUEST['category']['2']!='Y'
				...
				while($row_immunization = mysql_fetch_array($res_immunization))
				{
					...
					echo "<td>".show_date($row_immunization['medical_date']) ."</td></tr>";
					}
...
# ---------------------------------- Medical Alert ---------------- #
			...
			$sql_alert = "select * from STUDENT_MEDICAL_ALERTS where student_id='".$_SESSION['student_id']."'";
			$res_alert = mysql_query($sql_alert);
			$num_alert = mysql_num_rows($res_alert);

			if($_REQUEST['category']['2'] && $num_alert >= 1)
			{
				//------------------------------------------------------------------------------
				#echo "<br>";
				#echo $_REQUEST['category']['2']!='Y'
				...
				while($row_alert = mysql_fetch_array($res_alert))
				{
					if($row_alert['title'] !='')
					{
					...
					echo "<td width=79% align='justify'>".show_date($row_alert['alert_date']) ."</td></tr>";
					}
				 echo "<tr><td colspan=2 style=\"border-bottom:1px dashed #999999;\">&nbsp;</td></tr>";
...
# ---------------------------------- Nurse Visit Record ---------------- #
			...
			$sql_visit = "select * from STUDENT_MEDICAL_VISITS where student_id='".$_SESSION['student_id']."'";
			$res_visit = mysql_query($sql_visit);
			$num_visit = mysql_num_rows($res_visit);


			if($_REQUEST['category']['2'] && $num_visit >= 1)
			{
				//------------------------------------------------------------------------------
				#echo "<br>";
				#echo $_REQUEST['category']['2']!='Y'
				...
				while($row_visit = mysql_fetch_array($res_visit))
				{
					...
					{
					echo "<tr><td width=21% style='font-weight:bold'>Date:</td>";
					echo "<td width=79%>".show_date($row_visit['school_date']) ."</td></tr>";
					}
			...
			echo "<tr><td valign=top colspan=3>";
                       ...
                       #  if(User('PROFILE')=='admin')
                        $sql_comment = "SELECT ID,COMMENT_DATE,COMMENT,CONCAT(s.FIRST_NAME,' ',s.LAST_NAME)AS USER_NAME FROM STUDENT_MP_COMMENTS,STAFF s WHERE STUDENT_ID='".$_SESSION['student_id']."'  AND s.STAFF_ID=STUDENT_MP_COMMENTS.STAFF_ID";
                        #else
