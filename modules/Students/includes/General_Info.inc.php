...
unset($THIS_RET);
$RET = DBGet(DBQuery("SELECT s.FIRST_NAME,s.LAST_NAME,s.GENDER, e.ID,e.GRADE_ID,e.ENROLLMENT_CODE,e.START_DATE,e.DROP_CODE,e.END_DATE,e.END_DATE AS END,e.SCHOOL_ID,e.NEXT_SCHOOL,e.CALENDAR_ID FROM STUDENT_ENROLLMENT e,STUDENTS s WHERE e.STUDENT_ID='".UserStudentID()."' AND e.SYEAR='".UserSyear()."' AND e.STUDENT_ID=s.STUDENT_ID ORDER BY e.START_DATE"),$functions);
$add = true;
if(count($RET))
{
...
if($_REQUEST['student_id'] && $_REQUEST['student_id']!='new'){
	$end_date=DBGet(DBQuery("SELECT END_DATE FROM STUDENT_ENROLLMENT WHERE STUDENT_ID=".$_REQUEST['student_id']." AND SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'"));
