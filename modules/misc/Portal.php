...
switch (User('PROFILE'))
{
    case 'admin':
        ...
        AND NOT EXISTS(SELECT '' FROM ATTENDANCE_COMPLETED ac WHERE ac.SCHOOL_DATE=acc.SCHOOL_DATE AND ac.STAFF_ID=cp.TEACHER_ID AND ac.PERIOD_ID=cp.PERIOD_ID) AND cp.DOES_ATTENDANCE='Y' AND s.ID=acc.SCHOOL_ID ORDER BY cp.TITLE,acc.SCHOOL_DATE"),array('SCHOOL_DATE'=>'ProperDate'));
            if (count($RET))
            {
                echo '<p><font color=#FF0000><b>Warning!! - Teachers have missing attendance. Go to : Users -> Teacher Programs -> Missing Attendance</b></font>';
                echo '</p>';
            }*/

            $RET = DBGET(DBQuery("SELECT s.TITLE AS SCHOOL,acc.SCHOOL_DATE,cp.TITLE,cp.COURSE_PERIOD_ID,cp.TEACHER_ID " .
				...
				"INNER JOIN COURSE_PERIODS cp ON cp.MARKING_PERIOD_ID=mp.MARKING_PERIOD_ID AND cp.DOES_ATTENDANCE='Y' " .
				...
				" AND acc.SCHOOL_DATE>=SUBDATE('".date('Y-m-d',strtotime(DBDate()))."',7) " .
            ...
            if (count($RET))
            {
                ...
                echo '</p>';
                 ...
                 #ListOutput($RET,array('SCHOOL_DATE'=>'Date','TITLE'=>'Period -Teacher','SCHOOL'=>'School'),'Period','Periods',$link,array(),array('save'=>false,'search'=>false));
         ...
         $assignments_RET = DBGet(DBQuery( "SELECT ga.ASSIGNMENT_ID,ga.TITLE,ga.DESCRIPTION as COMMENT,ga.ASSIGNED_DATE,ga.DUE_DATE,ga.POINTS AS POINTS_POSSIBLE,at.TITLE AS CATEGORY
                                                 ...
                                                 ,GRADEBOOK_ASSIGNMENT_TYPES at
                                                  WHERE ga.ASSIGNMENT_ID NOT IN ".$GRADED_ASSIGNMENT." AND (ga.COURSE_PERIOD_ID='$course[COURSE_PERIOD_ID]' OR ga.COURSE_ID='$course[COURSE_ID]' AND ga.STAFF_ID='$staff_id') AND ga.MARKING_PERIOD_ID='".UserMP()."'
                                                   AND at.ASSIGNMENT_TYPE_ID=ga.ASSIGNMENT_TYPE_ID AND(  CURRENT_DATE>=ga.ASSIGNED_DATE OR CURRENT_DATE<=ga.ASSIGNED_DATE )AND (ga.DUE_DATE IS NULL OR CURRENT_DATE<=ga.DUE_DATE)
