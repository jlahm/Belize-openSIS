...
if($_REQUEST['search_modfunc'] || $_REQUEST['student_id'] || UserStudentID() || User('PROFILE')=='parent' || User('PROFILE')=='student')
{
    ...
    {
        "SELECT PERIOD_ID,TITLE FROM SCHOOL_PERIODS WHERE SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'";
 ...
 "SELECT DISTINCT SCHOOL_DATE,CONCAT('_',DATE_FORMAT(SCHOOL_DATE,'%Y%m%d')) AS SHORT_DATE FROM ATTENDANCE_CALENDAR WHERE SCHOOL_ID='".UserSchool()."' AND SCHOOL_DATE BETWEEN '".date('Y-m-d',strtotime($start_date))."' AND '".date('Y-m-d',strtotime($end_date))."' ORDER BY SCHOOL_DATE";
...
$sql = "SELECT ad.STATE_VALUE,ad.STUDENT_ID,SCHOOL_DATE,CONCAT('_',DATE_FORMAT(ad.SCHOOL_DATE,'%Y%m%d')) AS SHORT_DATE FROM ATTENDANCE_DAY ad,STUDENT_ENROLLMENT ssm WHERE ad.STUDENT_ID=ssm.STUDENT_ID AND '".date('Y-m-d',strtotime(DBDate()))."'>=ssm.START_DATE AND ssm.SCHOOL_ID='".UserSchool()."' AND SCHOOL_DATE BETWEEN '".date('Y-m-d',strtotime($start_date))."' AND '".date('Y-m-d',strtotime($end_date))."'";
    ...
    else
    {
         ...
         $sql = "SELECT ap.ATTENDANCE_CODE,ap.STUDENT_ID,ap.SCHOOL_DATE,CONCAT('_',DATE_FORMAT(ap.SCHOOL_DATE,'%Y%m%d')) AS SHORT_DATE FROM ATTENDANCE_PERIOD ap,STUDENT_ENROLLMENT ssm,STUDENTS s WHERE s.STUDENT_ID=ssm.STUDENT_ID AND ap.STUDENT_ID=ssm.STUDENT_ID AND ap.SCHOOL_DATE BETWEEN '".date('Y-m-d',strtotime($start_date))."' AND '".date('Y-m-d',strtotime($end_date))."'";
