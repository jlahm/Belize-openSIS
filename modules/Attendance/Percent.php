...
if(!$_REQUEST['modfunc'])
{
	...
	#		$student_days_possible = DBGet(DBQuery("SELECT ssm.GRADE_ID,ssm.CALENDAR_ID,'' AS DAYS_POSSIBLE, COUNT(ap.STUDENT_ID) AS ATTENDANCE_POSSIBLE, count(*) AS STUDENTS,'' AS PRESENT,'' AS ABSENT,'' AS ADA,'' AS AVERAGE_ATTENDANCE,'' AS AVERAGE_ABSENT FROM STUDENT_ENROLLMENT ssm,ATTENDANCE_CALENDAR ac,STUDENTS s,ATTENDANCE_PERIOD ap WHERE s.STUDENT_ID=ssm.STUDENT_ID AND ssm.STUDENT_ID=ap.STUDENT_ID  AND ssm.SYEAR='".UserSyear()."' AND ac.SYEAR=ssm.SYEAR AND ac.CALENDAR_ID=ssm.CALENDAR_ID AND ".($_REQUEST['_search_all_schools']!='Y'?"ssm.SCHOOL_ID='".UserSchool()."' AND ":'')." ssm.SCHOOL_ID=ac.SCHOOL_ID AND  ac.SCHOOL_DATE=ap.SCHOOL_DATE AND (ac.SCHOOL_DATE BETWEEN ssm.START_DATE AND ssm.END_DATE OR (ssm.END_DATE IS NULL AND ssm.START_DATE <= ac.SCHOOL_DATE)) AND ac.SCHOOL_DATE BETWEEN '".date('Y-m-d',strtotime($start_date))."' AND '".date('Y-m-d',strtotime($end_date))."' ".$extra2),array('GRADE_ID'=>'_make','STUDENTS'=>'_make','PRESENT'=>'_make','ABSENT'=>'_make','ADA'=>'_make','AVERAGE_ATTENDANCE'=>'_make','AVERAGE_ABSENT'=>'_make','DAYS_POSSIBLE'=>'_make'));
	...
	#	echo "SELECT ssm.GRADE_ID,ssm.CALENDAR_ID,COALESCE(sum(ad.STATE_VALUE-1)*-1,0) AS STATE_VALUE FROM ATTENDANCE_DAY ad,STUDENT_ENROLLMENT ssm,STUDENTS s".$extra['FROM']." WHERE s.STUDENT_ID=ssm.STUDENT_ID AND ad.STUDENT_ID=ssm.STUDENT_ID AND ssm.SYEAR='".UserSyear()."' AND ad.SYEAR=ssm.SYEAR AND ad.SCHOOL_DATE BETWEEN '$start_date' AND '$end_date' AND (ad.SCHOOL_DATE BETWEEN ssm.START_DATE AND ssm.END_DATE OR (ssm.END_DATE IS NULL AND ssm.START_DATE <= ad.SCHOOL_DATE)) ".$extra['WHERE'];
...
//		$student_days_absent = DBGet(DBQuery("SELECT ssm.GRADE_ID,ssm.CALENDAR_ID,COALESCE(sum(ad.STATE_VALUE-1)*-1,0) AS STATE_VALUE FROM ATTENDANCE_DAY ad,STUDENT_ENROLLMENT ssm,STUDENTS s".$extra['FROM']." WHERE s.STUDENT_ID=ssm.STUDENT_ID AND ad.STUDENT_ID=ssm.STUDENT_ID AND ssm.SYEAR='".UserSyear()."' AND ad.SYEAR=ssm.SYEAR AND ad.SCHOOL_DATE BETWEEN '$start_date' AND '$end_date' AND (ad.SCHOOL_DATE BETWEEN ssm.START_DATE AND ssm.END_DATE OR (ssm.END_DATE IS NULL AND ssm.START_DATE <= ad.SCHOOL_DATE)) ".$extra['WHERE']),array(''),array('GRADE_ID','CALENDAR_ID'));
	...
	#	echo "SELECT ssm.GRADE_ID,ssm.CALENDAR_ID,COALESCE(sum(ad.STATE_VALUE-1)*-1,0) AS STATE_VALUE FROM ATTENDANCE_DAY ad,STUDENT_ENROLLMENT ssm,STUDENTS s".$extra['FROM']." WHERE s.STUDENT_ID=ssm.STUDENT_ID AND ad.STUDENT_ID=ssm.STUDENT_ID AND ssm.SYEAR='".UserSyear()."' AND ad.SYEAR=ssm.SYEAR AND ad.SCHOOL_DATE BETWEEN '$start_date' AND '$end_date' AND (ad.SCHOOL_DATE BETWEEN ssm.START_DATE AND ssm.END_DATE OR (ssm.END_DATE IS NULL AND ssm.START_DATE <= ad.SCHOOL_DATE)) ".$extra['WHERE'];
	#	echo'<pre>';print_r($student_days_absent);echo'</pre>';
		$student_not_taken = DBGet(DBQuery("SELECT ssm.GRADE_ID,ac.CALENDAR_ID,COUNT(*) AS NOT_TAKEN FROM ATTENDANCE_CALENDAR ac".$extra['FROM']." INNER JOIN STUDENT_ENROLLMENT ssm ON ssm.SYEAR=ac.SYEAR AND ssm.SCHOOL_ID=ac.SCHOOL_ID AND ssm.CALENDAR_ID=ac.CALENDAR_ID AND ac.SCHOOL_DATE BETWEEN ssm.START_DATE AND COALESCE(ssm.END_DATE,CURDATE()) LEFT JOIN ATTENDANCE_DAY ad ON ad.SYEAR=ac.SYEAR AND ad.STUDENT_ID=ssm.STUDENT_ID AND ad.SCHOOL_DATE=ac.SCHOOL_DATE WHERE ssm.SYEAR=2010 AND ac.SCHOOL_DATE BETWEEN '$start_date' AND '$end_date' AND ad.STUDENT_ID IS NULL ".$extra['WHERE']),array(''),array('GRADE_ID','CALENDAR_ID'));
				
		#$student_days_present = DBGet(DBQuery("SELECT COUNT(*) AS PRESENT_BY_GREAD,SE.GRADE_ID FROM `ATTENDANCE_PERIOD` AP,STUDENT_ENROLLMENT SE WHERE AP.ATTENDANCE_CODE IN (3, 4, 5) AND SE.SCHOOL_ID=".UserSchool()." AND AP.SCHOOL_DATE BETWEEN '".date('Y-m-d',strtotime($start_date))."' AND '".date('Y-m-d',strtotime($end_date))."' AND AP.STUDENT_ID=SE.STUDENT_ID GROUP BY SE.GRADE_ID"));
		
		/*
		$ac_id = "";
		$sql_ac = "select * from attendance_codes where state_code='P'";
		$res_ac = mysql_query($sql_ac);
		while($row_ac = mysql_fetch_array($res_ac))
		{
			$ac_id.=$row_ac['id'].", "; 
		}
		$ac_id = substr($ac_id, 0, -2);
		*/
		
		
		
		//	$student_days_present = DBGet(DBQuery("SELECT COUNT(*) AS PRESENT_BY_GREAD,SE.GRADE_ID FROM `ATTENDANCE_PERIOD` AP,STUDENT_ENROLLMENT SE WHERE AP.ATTENDANCE_CODE IN (SELECT ID FROM ATTENDANCE_CODES WHERE STATE_CODE!='A') AND SE.SCHOOL_ID=".UserSchool()." AND AP.SCHOOL_DATE BETWEEN '".date('Y-m-d',strtotime($start_date))."' AND '".date('Y-m-d',strtotime($end_date))."' AND AP.STUDENT_ID=SE.STUDENT_ID AND SE.CALENDAR_ID=(SELECT CALENDAR_ID FROM ATTENDANCE_CALENDARS WHERE SCHOOL_ID=".UserSchool()." AND SYEAR=".UserSyear()." LIMIT 1) GROUP BY SE.GRADE_ID"));
	...
	#   To show Total Attendance column in the report uncomment the code below and comment out line 176.  
	#	$columns = array('GRADE_ID'=>_('Grade'),'STUDENTS'=>_('Students'),'DAYS_POSSIBLE'=>_('Days Possible'),'TOTAL_ATTENDANCE'=>_('Total Attendance'),'PRESENT'=>_('Present'),'ABSENT'=>_('Absent'),'ADA'=>_('ADA'),'AVERAGE_ATTENDANCE'=>_('Average Attendance'),'AVERAGE_ABSENT'=>_('Average Absent'));
	
	 
	#	 $columns = array('GRADE_ID'=>'Grade','STUDENTS'=>'Students','DAYS_POSSIBLE'=>'Days Possible','PRESENT'=>'Present','ABSENT'=>'Absent','ADA'=>'ADA','AVERAGE_ATTENDANCE'=>'Average Attendance','AVERAGE_ABSENT'=>'Average Absent');
...
*/
                       // echo '<pre>';print_r($student_days_present);echo '</pre>';
		    $present_by_gread = 0;
			/*
		    foreach($student_days_present as $student_present)
			{
				if($student_present['GRADE_ID']==$THIS_RET['GRADE_ID'])
			   	{
	             	$present_by_gread = $student_present['PRESENT_BY_GREAD'];
			    }
			}
			*/
		...
		case 'ADA':
			...
			$students = $value;
			$total_attn = ($dayespossible * $students);
			$ada = (($present/$total_attn)*100);
			$ada = round($ada, 2).'%'; */
			#$ada = round($present_by_gread*100/$dayespossible,2).'%';
			$present_by_gread = $student_days_present[$THIS_RET['GRADE_ID']][$THIS_RET['CALENDAR_ID']][1]['PRESENT_BY_GREAD'];
