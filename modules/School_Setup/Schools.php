...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='update' && (clean_param($_REQUEST['button'],PARAM_ALPHAMOD)=='Save' || clean_param($_REQUEST['button'],PARAM_ALPHAMOD)=='Update' || clean_param($_REQUEST['button'],PARAM_ALPHAMOD)==''))
{
	if(clean_param($_REQUEST['values'],PARAM_NOTAGS) && $_POST['values'] && User('PROFILE')=='admin')
	{
		...
		#		echo "UPDATE STAFF SET SCHOOLS=CONCAT(CONCAT(SCHOOLS,','),'$id') WHERE STAFF_ID='".User('STAFF_ID')."' AND SCHOOLS IS NOT NULL";
				...
				$sch_id = DBGet(DBQuery("SELECT MAX(ID) AS ID FROM SCHOOLS"));
				$sch_id = $sch_id[1]['ID'];
				
				DBQuery("INSERT INTO SYSTEM_PREFERENCE(school_id, full_day_minute, half_day_minute) VALUES (".$sch_id.", NULL, NULL)");
...
#	echo "</TABLE></FIELDSET>";
	...
	if(User('PROFILE')=='admin' && AllowEdit())
	{
	 	if($_REQUEST['new_school'])
		{
			DrawHeader('','',"<CENTER><INPUT TYPE=BUTTON name=button id=button class=btn_medium VALUE='Save' onclick='formcheck_school_setup_school();'></CENTER>");
		}
		else
		{
			...
			DrawHeader('','',"<CENTER><INPUT TYPE=BUTTON name=button id=button class=btn_medium VALUE='Update' onclick='formcheck_school_setup_school();'></CENTER>");
		}
	}
