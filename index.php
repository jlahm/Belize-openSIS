...
$maintain_RET=DBGet(DBQuery("SELECT SYSTEM_MAINTENANCE_SWITCH FROM SYSTEM_PREFERENCE_MISC LIMIT 1"));
	 $maintain=$maintain_RET[1];
	   $login_RET = DBGet(DBQuery("SELECT USERNAME,PROFILE,STAFF_ID,CURRENT_SCHOOL_ID,LAST_LOGIN,FIRST_NAME,LAST_NAME,PROFILE,IS_DISABLE,PROFILE_ID,FAILED_LOGIN FROM STAFF WHERE SYEAR='$DefaultSyear' AND UPPER(USERNAME)=UPPER('$username') AND UPPER(PASSWORD)=UPPER('$password')"));

    $student_RET = DBGet(DBQuery("SELECT s.USERNAME,s.STUDENT_ID,s.LAST_LOGIN,s.IS_DISABLE,s.FAILED_LOGIN FROM STUDENTS s,STUDENT_ENROLLMENT se WHERE UPPER(s.USERNAME)=UPPER('$username') AND UPPER(s.PASSWORD)=UPPER('$password') AND se.STUDENT_ID=s.STUDENT_ID AND se.SYEAR=$DefaultSyear AND CURRENT_DATE>=se.START_DATE AND (CURRENT_DATE<=se.END_DATE OR se.END_DATE IS NULL)"));
 ...
 if(($login && $login['PROFILE_ID']!=1)||$login['PROFILE_ID']==0){
  ...
  exit;
    ...
    {
        $admin_RET = DBGet(DBQuery("SELECT STAFF_ID FROM STAFF WHERE PROFILE='$username' AND SYEAR='$DefaultSyear' AND UPPER(PASSWORD)=UPPER('$password')"));  // Uid and Password Checking
        if($admin_RET)
        {
             $login_RET = DBGet(DBQuery("SELECT USERNAME,PROFILE,STAFF_ID,CURRENT_SCHOOL_ID,LAST_LOGIN,FIRST_NAME,LAST_NAME,PROFILE_ID,FAILED_LOGIN,IS_DISABLE FROM STAFF WHERE SYEAR='$DefaultSyear' AND UPPER(USERNAME)=UPPER('$username')"));
            $student_RET = DBGet(DBQuery("SELECT s.FIRST_NAME as FIRST_NAME,s.USERNAME,s.STUDENT_ID,s.LAST_LOGIN,s.FAILED_LOGIN FROM STUDENTS s,STUDENT_ENROLLMENT se WHERE UPPER(s.USERNAME)=UPPER('$username') AND se.STUDENT_ID=s.STUDENT_ID AND se.SYEAR=$DefaultSyear AND CURRENT_DATE>=se.START_DATE AND (CURRENT_DATE<=se.END_DATE OR se.END_DATE IS NULL)"));
        }
    }
...
//if($_REQUEST['USERNAME']!='' && $_REQUEST['PASSWORD']!='' && $value_min_id == $value_max_id)
		if(optional_param('USERNAME','',PARAM_RAW)!='' && optional_param('PASSWORD','',PARAM_RAW)!='' && $value_min_id == $value_max_id)
		{
			...
			DBQuery("INSERT INTO LOGIN_RECORDS (SYEAR,STAFF_ID,FIRST_NAME,LAST_NAME,PROFILE,USER_NAME,LOGIN_TIME,FAILLOG_COUNT,IP_ADDRESS,STATUS,SCHOOL_ID) values('$DefaultSyear','$_SESSION[STAFF_ID]','$_SESSION[FIRST_NAME]','$_SESSION[LAST_NAME]','$_SESSION[PROFILE]','$_SESSION[USERNAME]','$date','$_SESSION[FAILED_LOGIN]','$ip','Success',$_SESSION[CURRENT_SCHOOL_ID])");
		...
		if ($ad_f_cnt && $ad_f_cnt!=0 && $failed_login>$ad_f_cnt && $profile_id!=1)
		{
		  ...
		  DBQuery("UPDATE STAFF SET IS_DISABLE='Y' WHERE STAFF_ID='".$_SESSION['STAFF_ID']."' AND SYEAR='$DefaultSyear' AND PROFILE_ID!=1");
...
";
    ...
    {  /* cleaning using other parameters other than ALPHAEXT is not working----*/
        DBQuery("UPDATE STAFF SET FAILED_LOGIN=".db_case(array('FAILED_LOGIN',"''",'1','FAILED_LOGIN+1'))." WHERE UPPER(USERNAME)=UPPER('".optional_param('USERNAME', 0, PARAM_ALPHAEXT)."') AND SYEAR='$DefaultSyear'");
	...
	#	DBQuery("UPDATE STUDENTS SET FAILED_LOGIN=".db_case(array('FAILED_LOGIN',"''",'1','FAILED_LOGIN+1'))." WHERE UPPER(USERNAME)=UPPER('$_REQUEST[USERNAME]')");
		...
		DBQuery("INSERT INTO LOGIN_RECORDS (USER_NAME,FAILLOG_TIME,IP_ADDRESS,SYEAR,STATUS) values('".optional_param('USERNAME','',PARAM_ALPHAEXT)."','$faillog_time','$ip','$DefaultSyear','Failed')"); 
